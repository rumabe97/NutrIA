...
/**
 * Scores how well `abbreviation` matches `value`. Returns a number between 0 and 1 — 1 is a perfect
 * continuous match and 0 means no match. Optional `aliases` are appended to the searchable surface
 * so a keyword can score the item without appearing in its visible label.
 *
 * Algorithm based on the `command-score` package; lowercasing is hoisted to the outer call so the
 * recursive inner function can stay cheap.
 */
export function commandScore(value: string, abbreviation: string, aliases?: ReadonlyArray<string>): number {
  const searchable = aliases && aliases.length > 0 ? `${value} ${aliases.join(' ')}` : value;

  return commandScoreInner(searchable, abbreviation, formatInput(searchable), formatInput(abbreviation), 0, 0, {});
}
