...
**Estado de P0-3 a 2026-09-25**: la rama `agent/legal-a/backend` (`PROMPT_VERSION` 4.0.0, sin fusionar) aplica la salida (b): el prompt ya no lleva texto escrito por la persona, ni alergias sin resolver, ni halal/kósher (que pasan a aplicarse en código). **Queda abierto**: la forma de comer `gluten_free`/`lactose_free` y los objetivos siguen llegando a modelos gratuitos que pueden entrenar con ellos; son pocos datos, sin identificadores, pero `gluten_free` revela una celiaquía o una intolerancia. Cierra P0-3 del todo la salida (a) —proveedores con contrato y sin entrenamiento— o enviar esas dos formas de comer también como exclusión de ingredientes y no como etiqueta, igual que halal/kósher.
