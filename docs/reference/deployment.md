...
| `AI_PROVIDER` | `openrouter` in production once `legal` has cleared the provider (`0064`): OpenRouter directly, paid models that never train on what they are sent. Needs `OPENROUTER_API_KEY`, and `AI_BASE_URL` **empty** — set, it would send the OpenRouter key to wherever it points. `stub` generates from the library alone (what production runs until then). `google` calls Gemini directly with `GOOGLE_API_KEY`; `omniroute` goes through a gateway and needs `AI_BASE_URL`, `OMNIROUTE_API_KEY` and `OMNIROUTE_MODEL` — both routes are in [`ai-gateway.md`](./ai-gateway.md) |
| `OPENROUTER_API_KEY`, `AI_MODEL`, `AI_FALLBACK_MODELS`, `AI_REASONING_EFFORT` | `openrouter` only (`0064`): the key from [`ai-gateway.md`](./ai-gateway.md) §0; `deepseek/deepseek-v4.1-flash` (also the default when empty); `minimax/minimax-m3` (comma-separated `vendor/model` ids, never `:free` — refused at boot); `low` (`none`, `minimal`, `low`, `medium` or `high`; empty leaves the model's default). All ignored by every other provider |
