...
| `profile-consent.e2e-spec.ts` | The explicit profile consent (`docs/legal/textos/05-consentimientos-cliente.md` § A, P0-2): without it, the onboarding steps that collect what it covers (`goal`, `body-activity`, `allergies`), `PATCH /profile/goal`, a height on `PATCH /profile`, `PUT /safety/restrictions`, `POST /progress/weight` and generation itself all refuse with 409 `PROFILE_CONSENT_REQUIRED` and store nothing — and accept once it is given. An account that finished onboarding before this change is asked for it again, its data untouched, and generation refuses until it is given. Withdrawing it deletes what it covered — allergies, intolerances, custom allergens, dietary patterns, the goal, the height, every logged weight — in one transaction, read back from the tables themselves, reopens onboarding at those same three steps, leaves another account's data untouched, and generation refuses again. The minimum age (18, 2026-09-25): a birth date under it is refused with 422 `UNDER_MINIMUM_AGE` on both `PATCH /onboarding` and `PATCH /profile` and stores nothing, and exactly 18 today is accepted. |
