...
| `professionals.e2e-spec.ts` | An account becomes a professional only by the owner's act, with a collegiate number (`0059`): a sign-up body, the account update and a profile body carrying the words change nothing, the owner's list names no client, the switch fails off, and taking the grant back takes access with it. Its own `GET /users/me` carries `professional`: false for an ordinary account, false for a grant while the switch is off, true once both hold, false on the very next request once either is taken away — and a real grant with the switch off is a 404 on `/care` whatever the request body claims about itself. Forging it — the account update field by field (each its own exact refusal) and all at once, every `profile` PATCH route with every field at once, the owner's own grant route — opens no route the professional side of care or `/admin` owns and leaves no row in `professionals`. |
...
| `custom-allergens.e2e-spec.ts` | A free-text allergy that matched the catalogue is enforced exactly as a listed one; one that did not is stored and surfaced as unenforceable — and, since P0-3 (2026-09-25), never named to the model, mitigated in code instead. A declared religious pattern (`halal`) is enforced in code and never named either, and a check-in comment fed to a redo does not reach the model. |
| `profile-consent.e2e-spec.ts` | The explicit profile consent (`docs/legal/textos/05-consentimientos-cliente.md` § A, P0-2): without it, the onboarding steps that collect what it covers (`goal`, `body-activity`, `allergies`), `PATCH /profile/goal`, a height on `PATCH /profile`, `PUT /safety/restrictions`, `POST /progress/weight` and generation itself all refuse with 409 `PROFILE_CONSENT_REQUIRED` and store nothing — and accept once it is given. An account that finished onboarding before this change is asked for it again, its data untouched, and generation refuses until it is given. `GET /profile/consent` names the current version and answers false until `PUT` with it, which any other version — 422 `INVALID_INPUT` — cannot do. Withdrawing (`DELETE`) deletes what it covered — allergies, intolerances, custom allergens, dietary patterns, the goal, the height, every logged weight — in one transaction, read back from the tables themselves, reopens onboarding at those same three steps, leaves another account's data untouched, generation refuses again, and a second withdrawal is idempotent. The minimum age (18, 2026-09-25): a birth date under it is refused with 422 `UNDER_MINIMUM_AGE` on both `PATCH /onboarding` and `PATCH /profile` and stores nothing, and exactly 18 today is accepted. |
